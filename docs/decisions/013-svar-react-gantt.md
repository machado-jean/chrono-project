...
- grid exposto à tecnologia assistiva, enquanto o Chrono Project fornece um painel
...
O Chrono Project converte `Task` e `TaskDependency` para uma projeção transitória;
...
O Chrono Project persiste movimento, término e conclusão; em falha, força nova
...
O contrato `date-only` do Chrono Project trata o fim como inclusivo, enquanto o
